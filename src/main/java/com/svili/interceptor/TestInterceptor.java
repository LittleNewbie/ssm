...
public class TestInterceptor implements HandlerInterceptor {
	...
	/**
	 * preHandle方法是进行处理器拦截用的，顾名思义，该方法将在Controller处理之前进行调用.</br>
	 * SpringMVC中的Interceptor拦截器是链式的，可以同时存在
	 * 多个Interceptor，然后SpringMVC会根据声明的前后顺序一个接一个的执行，
	 * 而且所有的Interceptor中的preHandle方法都会在 Controller方法调用之前调用.</br>
	...
	/**
	 * 该方法将在整个请求完成之后,也就是DispatcherServlet渲染了视图之后执行, 这个方法的主要作用是用于清理资源的.</br>
